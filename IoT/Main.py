...
# SG_highest_alert['Community_score'] = ''
# SG_highest_alert['harmless'] = ''
# SG_highest_alert['malicious'] = ''
# SG_highest_alert['suspicious'] = ''
# SG_highest_alert['undetected'] = ''
# SG_highest_alert['timeout'] = ''
# SG_highest_alert['Engines'] = ''
#
# ip_address = SG_highest_alert['SrcIP'].unique()
# count = 0
# total_ip = len(ip_address)
#
# for ip in tqdm(ip_address, desc = "virus total checks"):
...
#     #output_filename = os.path.splitext(file)[0] + "_VT_enriched_" + suffix + ".csv"
#     SG_highest_alert.to_csv("VT_matched.csv" + "_" + str(datetime.now().strftime('%Y_%m_%d_%H_%M_%S')) + ".csv", index=False, index=False)

#     count += 1
#     os.system('cls')
#     print("Processed {} out of {} IPs".format(count, total_ip))
#
...
for j in tqdm(OS_search['SrcIP'], desc = "Open Search API checks"):
